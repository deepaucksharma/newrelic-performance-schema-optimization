...
variable "vpc_cidr" {
  description = <<-EOT
    CIDR block allowed for Lambda egress.
    • "" (default) → use the VPC's primary CIDR.
    • Needed only when the DB is reachable *solely* via a secondary CIDR.
      Example: 10.1.0.0/16 in a multi-CIDR VPC.
  EOT
  type        = string
  default     = ""
}
